...
if (!isset($item->id)) {
    ...
    $clockCmbOffi = 'select-disable ctr-readonly';
}else{
    // $clockCmbOffi = '';
    $clockCmbOffi = 'select-disable ctr-readonly';
}
...
<div class="edit-content">
    {{-- <div class="row mb-2">
        <div class="col-8 d-flex justify-content-start">
            <button type="button" class="btn btn-blue"  onclick="Save(`{{route('u03.saveInfo')}}`)">保存</button>
        <button type="button" class="btn btn-outline-primary"  onclick="BackPrev(`{{route('u02')}}`,`{{route('u03.4')}}`)">キャンセル</button>
        <button type="button" class="btn btn-warning"  onclick="Approval(`{{$url}}`,'btnHD')">HD確認</button>
        </div>
        <div class="col-4 d-flex justify-content-end">
            <button type="button" class="btn btn-success float-end" target="_blank" onclick="window.open('{{route('u03.4')}}','_blank')">入職者申請状況</button>
        </div>
    </div> --}}
    <div>
        <button type="button" class="btn btn-blue  mb-2"  onclick="Save(`{{route('u03.saveInfo')}}`)">保存</button>
        <button type="button" class="btn btn-outline-primary  mb-2"  onclick="BackPrev(`{{route('u02')}}`,`{{route('u03.4')}}`)">キャンセル</button>
        <button type="button" class="btn btn-warning  mb-2"  onclick="Approval(`{{$url}}`,'btnHD')">HD確認</button>
        <button type="button" class="btn btn-success float-sm-end float-none mb-2" target="_blank" onclick="window.open('{{route('u03.4')}}','_blank')">入職者申請状況</button>
    </div>
                ...
                <table class="table table-bordered table-border-color">
                    <thead class="sticky-top top--1">
                        <tr id="" class="tr-head bg-title text-center p-1">
                            <th scope="col" class="mw-c-150">フロー</th>
                            <th scope="col" class="mw-c-200">氏名</th>
                            <th scope="col" class="mw-c-300">更新日時</th>
                    ...
                    <tbody id="">
                        <tr>
                            ...
                            <td class="text-center w-c-50 border-0 border-bottom border-top border-white">
                                @can('isAdmin')
                                @if($item && $item->status_user === CommonConst::STATUS_REJECT || empty($item))
                                <button type="button" class="btn btn-blue" onclick="Approval(`{{$url}}`,'author')">送信</button>
                                @endif
                                @endcan
                            </td>
                            <td class="text-center w-c-50 border-0 border-bottom border-top border-white">
                        ...
                        <tr>
                            ...
                            <td>
                                <label id="lbApplicantDate" for="applicant_date">
                        ...
                        <tr>
                            ...
                            <td class="text-center w-c-50 border-0 border-bottom border-top border-white">
                                {{-- @can('isAdmin')
                                @if($item && $item->status_user === CommonConst::STATUS_APPROVAL_1) --}}
                                <button type="button" class="btn btn-blue" onclick="Approval(`{{$url}}`,'admin1')">承認</button>
                                {{-- @endif
                                @endcan --}}
                            </td>
                            <td class="text-center w-c-50 border-0 border-bottom border-top border-white">
                                {{-- @can('isAdmin')
                                @if($item && $item->status_user === CommonConst::STATUS_APPROVAL_1) --}}
                                <button type="button" class="btn btn-danger" onclick="Approval(`{{$url}}`,'admin1Reject')">否認</button>
                                {{-- @endif
                                @endcan --}}
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td class="text-center w-c-50 border-0 border-bottom border-top border-white">
                                {{-- @can('isAdmin') --}}
                                {{-- @if($item && $item->status_user === CommonConst::STATUS_APPROVAL_2) --}}
                                <button type="button" class="btn btn-blue" onclick="Approval(`{{$url}}`,'admin2')">承認</button>
                                {{-- @endif --}}
                                {{-- @endcan --}}
                            </td>
                            <td class="text-center w-c-50 border-0 border-bottom border-top border-white">
                                {{-- @can('isAdmin') --}}
                                {{-- @if($item && $item->status_user === CommonConst::STATUS_APPROVAL_2) --}}
                                <button type="button" class="btn btn-danger" onclick="Approval(`{{$url}}`,'admin2Reject')">否認</button>
                                {{-- @endif --}}
                                {{-- @endcan --}}
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td class="text-center w-c-50 border-0 border-bottom border-top border-white">
                                @can('isAdmin')
                                @if($item && $item->status_user === CommonConst::STATUS_APPROVAL_HD)
                                <button type="button" class="btn btn-blue" onclick="Approval(`{{$url}}`,'hd')">承認</button>
                                @endif
                                @endcan
                            </td>
                            <td class="text-center w-c-50 border-0 border-bottom border-top border-white">
                            @can('isAdmin')
                                @if($item && $item->status_user === CommonConst::STATUS_APPROVAL_HD)
                                <button type="button" class="btn btn-danger" onclick="Approval(`{{$url}}`,'hdReject')">否認</button>
                                @endif
                                @endcan
                            </td>
                        </tr>
    ...
    <div class="row mb-2">
        <div class="col-xl-2 col-md-4">
            ...
            <input type="date" name="txtDate_join" autocomplete="off" class="form-control border-radius-none" maxlength="10" value="{{ !empty($item) && $item->date_join != '' ? date(CommonConst::DATE_FORMAT_2, strtotime($item->date_join)) : '' }}">
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-xl-2 col-md-4 mb-2">
            ...
            <input name="txtFirstName" type="text" autocomplete="off" class="form-control border-radius-none" maxlength="30" value="{{old('txtFirstName', $item->first_name  ?? '')}}">
        </div>
        <div class="col-xl-2 col-md-4 mb-2">
            <div class="bg-title text-center p-1"><span class="note">*</span><label>氏名（名）</label></div>
            <input name="txtLastName" type="text" autocomplete="off" class="form-control border-radius-none" maxlength="30" value="{{old('txtLastName', $item->last_name  ?? '')}}">
        </div>
        <div class="col-xl-2 col-md-4 mb-2">
            <div class="bg-title text-center p-1"><span class="note">*</span><label>フリガナ（姓）</label></div>
            <input name="txtFirstFurigana" id="txtFirstFurigana" type="text" autocomplete="off" class="form-control border-radius-none" maxlength="30" value="{{old('txtFirstFurigana', $item->first_furigana  ?? '')}}">
        </div>
        <div class="col-xl-2 col-md-4 mb-2">
            <div class="bg-title text-center p-1"><span class="note">*</span><label>フリガナ（名）</label></div>
            <input name="txtLastFurigana" type="text" autocomplete="off" class="form-control border-radius-none" maxlength="30" value="{{old('txtLastFurigana', $item->last_furigana  ?? '')}}">
        </div>
        <div class="col-xl-2 col-md-4 mb-2">
            <div class="bg-title text-center p-1"><span class="note">*</span><label>ステータス</label></div>
            <select name="cmbWorkStatus" id="cmbWorkStatus" class="form-select border-radius-none">
                <option></option>
    ...
    <div class="row mb-4">
        ...
        <div class="col-xl-3 col-md-6 mb-2">
            ...
            <select name="cmbOffice_cd" id="cmbOffice" class="form-select border-radius-none {{$clockCmbOffi}}" >
                <option></option>
                @foreach ($ofces as $o)
        ...
        <div class="col-xl-2 col-md-4 mb-2">
            ...
            <input type="text" name="users_number" autocomplete="off" class="form-control border-radius-none allow_numeric pad-0 {{$disabled}}" maxlength="7" value="{{ $item->users_number  ?? ''}}">
        </div>
    </div>
    ...
    <div class="row mb-4">
                ...
                <div class="legend-body">
                    <div class="row ms-1" id="ListProItem">
                        ...
                        <div class="form-check col-4 col-md-3 col-xl-2 mb-2">
                            ...
                            @foreach ($arrPro as $idPro)
                                {{$idPro == $prod->id  ? 'checked value=1' : 'value' }}
                            @endforeach
                                id="prod_{{ $loop->iteration }}" name="prod_{{$prod->id}}">
                            <label class="form-check-label" for="prod_{{$prod->id}}" id="lbprod_{{ $prod->id }}">{{ $prod->name }}</label>
        ...
        <div class="col-xl-2 col-md-4 mb-0 mt-auto">
            ...
            <input type="text" id="txtCompany_mobile_number" name="txtCompany_mobile_number" autocomplete="off" class="form-control border-radius-none allow_numeric {{$disabledPhone}}" maxlength="20" value="{{ $item->company_mobile_number  ?? ''}}">
        </div>
    </div>
    ...
    <div class="row mb-4">
        <div class="col-xl-3 col-md-4 mb-2">
            ...
            <div class="d-flex">
                <input readonly id="txtResume" name="txtResume" class="cursor-pointer form-control border-radius-none border-right-0"  value="{{ !empty($upInfUser['type1']) ? $upInfUser['type1'][0]['file_name'] : '' }}" />
                <fieldset class="legend-box" style="padding-right:0; padding: bottom 0;padding-left: 0rem !important;" >
                    <div class="legend-body" style="padding-top: 0.75rem!important;">
                        <div class="d-flex">
                            <a target="_blank" id="linkResume" href="{{ !empty($upInfUser['type1']) ? asset($upInfUser['type1'][0]['path']) : '' }}" class="">
                                <input type="hidden" id="curResume" name="curResume" value="{{ !empty($upInfUser['type1']) ? 1 : 0 }}">
                            </a>
                            ...
                            <button class="btn-none h6 mt-auto ms-auto" type="button" onclick="return ShowFileDialog('Resume')">
                                <i class="bi bi-paperclip">FILEUP</i>
                            </button>
                        </div>
                    </div>
                </fieldset>
    ...
    <div class="row col-xl-6 col-md-8 mb-4">
        ...
        <div class="ms-4 mt-2" id="">
            <div class="d-flex align-items-center">
                <div class="form-check">
                    <input name="manager" id="manager" type="checkbox" class="form-check-input" onchange="OnCheckManager(this)" value="{{old('manager', $item->manager  ?? '')}}" {{ $item && $item->manager == '1' ? "disabled" : '' }} {{ $item ? ($item->manager == '1' ? 'checked' : '') : '' }}>
        ...
        <div class="{{ $mtb2_l4 }}">
            <div class="d-flex align-items-center">
                <div class="form-check">
                    <input name="manage_position" id="manage_position" class="form-check-input" type="checkbox" onchange="Oncheck(this)" value="{{old('manage_position', $item->manage_position  ?? '')}}" {{ $item ? ($item->manage_position == '1' ? 'checked' : '') : '' }}>
        ...
        <div id="rowFlg1" class="{{ $mtb2_l4 }}" {{ $item && $ofce->flg1 != '1' ? $hidden_cb : '' }}>
            <div class="d-flex align-items-center">
                <div class="form-check">
                    <input name="flg1" id="flg1" class="form-check-input" type="checkbox" onchange="Oncheck(this)" value="{{old('flg1', $item->flg1  ?? '')}}" {{ $item ? ($item->flg1 == '1' ? 'checked' : '') : '' }}>
        ...
        <div class="{{ $mtb2_l4 }}">
            <div class="d-flex align-items-center">
                <div class="form-check">
                    <input name="manage_business" id="manage_business" class="form-check-input" type="checkbox" onchange="Oncheck(this)" value="{{old('manage_business', $item->manage_business  ?? '')}}" {{ $item ? ($item->manage_business == '1' ? 'checked' : '') : '' }}>
        ...
        <div id="rowAccountant" class="{{ $mtb2_l4 }}" {{ $item && $ofce->flg1 != '1' ? $hidden_cb : '' }}>
            <div class="d-flex align-items-center">
                <div class="form-check">
                    <input name="accountant" id="accountant" class="form-check-input" type="checkbox" onchange="Oncheck(this)" value="{{old('accountant', $item->accountant  ?? '')}}" {{ $item ? ($item->accountant == '1' ? 'checked' : '') : '' }}>
        ...
        <div class="{{ $mtb2_l4 }}">
            <div class="d-flex align-items-center">
                <div class="form-check">
                    <input name="company_car" id="company_car" class="form-check-input" type="checkbox" onchange="OncheckCompanyCar(this)" value="{{old('company_car', $item->company_car  ?? '')}}" {{ $item ? ($item->company_car == '1' ? 'checked' : '') : '' }}>
        ...
        <div id="rowFlg3" class="{{ $mtb2_l4 }}" {{ $item && $ofce->flg3 != '1' ? $hidden_cb : '' }}>
            <div class="d-flex align-items-center">
                <div class="form-check">
                    <input name="flg3" id="flg3" class="form-check-input" type="checkbox" onchange="Oncheck(this)" value="{{old('flg3', $item->flg3  ?? '')}}" {{ $item ? ($item->flg3 == '1' ? 'checked' : '') : '' }}>
                    <label class="form-check-label" for="flg3">自家用車通勤33</label>
                </div>
            </div>
        </div>
        ...
        <div id="rowFlg4" class="{{ $mtb2_l4 }}" {{ $item && $ofce->flg4 != '1' ? $hidden_cb : '' }}>
            <div class="d-flex align-items-center">
                <div class="form-check">
                    <input name="flg4" id="flg4" class="form-check-input" type="checkbox" onchange="Oncheck(this)" value="{{old('flg4', $item->flg4  ?? '')}}" {{ $item ? ($item->flg4 == '1' ? 'checked' : '') : '' }}>
        ...
        <div id="rowFlg5" class="{{ $mtb2_l4 }}" {{ $item && $ofce->flg5 != '1' ? $hidden_cb : '' }}>
            <div class="d-flex align-items-center">
                <div class="form-check">
                    <input name="flg5" id="flg5" class="form-check-input" type="checkbox" onchange="Oncheck(this)" value="{{old('flg5', $item->flg5  ?? '')}}" {{ $item ? ($item->flg5 == '1' ? 'checked' : '') : '' }}>
        ...
        <div id="rowFlg6" class="{{ $mtb2_l4 }}" {{ $item ? ($item->flg6 != '1' ? $hidden_cb : '') : '' }}>
            <div class="d-flex align-items-center">
                <div class="form-check">
                    <input name="flg6" id="flg6" class="form-check-input" type="checkbox" onchange="checkFlg6(this)" value="{{old('flg6', $item->flg6  ?? '')}}" {{ $item ? ($item->flg6 == '1' && $item->manager != '1' ? 'checked' : '') : '' }}>
        ...
        <div id="rowFlg7" class="{{ $mtb2_l4 }}" {{ $item && $ofce->flg7 != '1' ? $hidden_cb : '' }}>
            <div class="d-flex align-items-center">
                <div class="form-check">
                    <input name="flg7" id="flg7" class="form-check-input" type="checkbox" onchange="Oncheck(this)" value="{{old('flg7', $item->flg7  ?? '')}}" {{ $item ? ($item->flg7 == '1' ? 'checked' : '') : '' }}>
        ...
        <div id="rowFlg8" class="{{ $mtb2_l4 }}" {{ $item && $ofce->flg8 != '1' ? $hidden_cb : '' }}>
            <div class="d-flex align-items-center">
                <div class="form-check">
                    <input name="flg8" id="flg8" class="form-check-input" type="checkbox" onchange="Oncheck(this)" value="{{old('flg8', $item->flg8  ?? '')}}" {{ $item ? ($item->flg8 == '1' ? 'checked' : '') : '' }}>
        ...
        <div id="rowFlg9" class="{{ $mtb2_l4 }}" {{ $item && $ofce->flg9 != '1' ? $hidden_cb : '' }}>
            <div class="d-flex align-items-center">
                <div class="form-check">
                    <input name="flg9" id="flg9" class="form-check-input" type="checkbox" onchange="Oncheck(this)" value="{{old('flg9', $item->flg9  ?? '')}}" {{ $item ? ($item->flg9 == '1' ? 'checked' : '') : '' }}>
    ...
    <fieldset class="legend-box">
        ...
        <div class="legend-body" id="ListUpOffice">
            ...
            <div class="form-check mb-2">
                <input class="form-check-input" type="checkbox" onchange="Oncheck(this)" @foreach ($arrUpOffi as $idUpOffi) {{$idUpOffi == $upOfice->id  ? 'checked' : '' }} @endforeach value="1" id="up_offi_{{$loop->iteration}}" name="up_offi_{{$upOfice->id}}">
...
<div class="row mb-2">
    ...
    <div class="col mb-2 mt-2">
        <button type="button" class="btn btn-blue ms-4"  onclick="UpdateLeter(`{{route('u03.saveInfo')}}`,`{{route('u03.2')}}`)">詳細</button>
...
<div class="row mb-2">
    ...
    <div class="co mb-2 mt-2">
        <button type="button" class="btn btn-blue ms-4"  onclick="UpdateLeter(`{{route('u03.saveInfo')}}`,`{{route('u03.3')}}`)">詳細</button>
...
<div class="row mb-2">
    ...
    <div class="col-xl-3 col-md-6 mb-2">
        ...
        @if(!empty($item) || ($item && $item->status_user != CommonConst::STATUS_CREATOR))
            <span class="note">*</span>
            @endif<label>（外国人雇用）雇用条件書</label></div>
        <div class="d-flex">
            <input readonly id="txtConditions" name="txtConditions" class="cursor-pointer form-control border-radius-none border-right-0"  value="{{ !empty($upInfUser['type2']) ? $upInfUser['type2'][0]['file_name'] : '' }}" />
            <fieldset class="legend-box" style="padding-right:0; padding: bottom 0;padding-left: 0rem !important;" >
                <div class="legend-body" style="padding-top: 0.75rem!important;">
                    <div class="d-flex">
                        <a target="_blank" id="linkConditions" href="{{ !empty($upInfUser['type2']) ? asset($upInfUser['type2'][0]['path']) : '' }}" class="">
                            <input type="hidden" id="curConditions" name="curConditions" value="{{ !empty($upInfUser['type2']) ? 1 : 0 }}">
                        </a>
                        ...
                        <button class="btn-none h6 mt-auto ms-auto" type="button" onclick="return ShowFileDialog('Conditions')">
...
<div class="row mb-2">
    <div class="col-xl-3 col-md-6 mb-2">
        ...
        @if(!empty($item) || ($item && $item->status_user != CommonConst::STATUS_CREATOR))
            <span class="note">*</span>
            @endif<label>（外国人雇用）徴収費用の説明書</label></div>
        <div class="d-flex">
            <input readonly id="txtCollectionFees" name="txtCollectionFees" class="cursor-pointer form-control border-radius-none border-right-0"  value="{{ !empty($upInfUser['type3']) ? $upInfUser['type3'][0]['file_name'] : '' }}" />
            <fieldset class="legend-box" style="padding-right:0; padding: bottom 0;padding-left: 0rem !important;" >
                <div class="legend-body" style="padding-top: 0.75rem!important;">
                    <div class="d-flex">
                        <a target="_blank" id="linkCollectionFees" href="{{ !empty($upInfUser['type3']) ? asset($upInfUser['type3'][0]['path']) : '' }}" class="">
                            <input type="hidden" id="curCollectionFees" name="curCollectionFees" value="{{ !empty($upInfUser['type3']) ? 1 : 0 }}">
                        </a>
                        ...
                        <button class="btn-none h6 mt-auto ms-auto" type="button" onclick="return ShowFileDialog('CollectionFees')">
                            <i class="bi bi-paperclip"></i>
                        </button>
                    </div>
                </div>
...
<div class="row">
    <div class="col-xl-3 col-md-6 mb-2">
        ...
        <div class="d-flex">
            <input readonly id="txtContract" name="txtContract" class="cursor-pointer form-control border-radius-none border-right-0"  value="{{ !empty($upInfUser['type4']) ? $upInfUser['type4'][0]['file_name'] : '' }}" />
            <fieldset class="legend-box" style="padding-right:0; padding: bottom 0;padding-left: 0rem !important;" >
                <div class="legend-body" style="padding-top: 0.75rem!important;">
                    <div class="d-flex">
                        <a target="_blank" id="linkContract" href="{{ !empty($upInfUser['type4']) ? asset($upInfUser['type4'][0]['path']) : '' }}" class="">
                            <input type="hidden" id="curContract" name="curContract" value="{{ !empty($upInfUser['type4']) ? 1 : 0 }}">
                        </a>
                        ...
                        <button class="btn-none h6 mt-auto ms-auto" type="button" onclick="return ShowFileDialog('Contract')">
                            <i class="bi bi-papercli6"></i>
                        </button>
                    </div>
                </div>
            </fieldset>
