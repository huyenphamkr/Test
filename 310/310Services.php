...
--------------------------------------------------------------------------------30-----------------------------------------------------------------------------------------
//u0310 contract info 
public function findContractInfo($id)
{
    $query = DB::table('contract_info_users as ci')
    ->leftJoin('procedures_users as p', 'p.info_users_cd', '=', 'ci.info_users_cd')
    ->leftJoin('info_users as i', 'i.id', '=', 'p.U03-10_id')
    ->leftJoin('info_users as iu', 'iu.id', '=', 'ci.info_users_cd')
    ->leftJoin('mst_offices as o', 'o.office_cd', '=', 'i.office_cd')
    ->where('ci.info_users_cd', '=', $id)
   // ->where('ci.ver_his', '=', '0')
    ->select(
        'ci.id as id',
        'ci.*',
        'i.*',
        'p.U03-10_date as datePro',
        'p.U03-10_id as namePro',
        'iu.first_name as firstNameU310',
        'iu.last_name as lastNameU310',
        'o.*',
    );
    return $query->first();
} 

public function saveContractInfo($data){
    // $idCont = $data['hidIdCon'];
    $id = $data['idInfo'];
    $dataToSave = [
        'U03-10_date' =>now(),
    ];
  
    $pro = ProceduresUsers::where('info_users_cd',$id)->first();
if($pro){
    $dataToSave['info_users_cd'] =  $id;
    $dataToSave['U03-10_id'] = $id;
    $dataToSave['revision'] = (int)$pro->revision + 1;
    $dataToSave['updated_id'] = 2;
    // $dataToSave['updated_id'] = Auth::id();
} else {
    //insert
    $dataToSave['info_users_cd'] =  $id;
...
//$contract = ProceduresUsers::updateOrCreate(['info_users_cd' => $id], $dataToSave);
$contract = ProceduresUsers::where('info_users_cd',$id)->update($dataToSave);
return $id;
}
